...
# modified slightly and named assemblathon_stats_ex2.pl JBH
#    this adds more output to standard report
...
my $min_scaff_len; # if set, sequences less than this length will be excluded from the analysis (part of _ex2) 
my $min_scaff_str; # if min_scaff_len is set, then we show this in header of analysis with this str
...
# set defaults
# change n_limit from 25 to 10 for the asmstats.pl named script
$limit = 1000000000 if (!$limit);
$n_limit = 10       if (!$n_limit);
$min_scaff_len = 0  if (!$min_scaff_len);
$min_scaff_str = "";
...
print "\n---------------- Information for assembly \'$file\' $min_scaff_str----------------\n\n";
...
sub process_FASTA{
	...
	while(my $entry = $fasta->nextEntry){
        ...
        if ($length < $min_scaff_len){ # skip this scaffold if it isn't our minimum length
            next;
        }
		...
		# if there are not at least 10 consecutive Ns in the sequence we need to split it into contigs
		...
		if ($seq =~ m/N{$n_limit}/){
			...
			foreach my $contig (split(/N{$n_limit,}/, $seq)){  # 06Feb2017 replace 25 with $n_limit
				$scaffolded_contigs++;
			...
			# must be here if the scaffold is actually just a contig (or is a scaffold with < 10 Ns)
...
sub sequence_statistics{
	...
	if($type eq 'intermediate'){
		...
		foreach my $scaffold (@{$data{scaffold}{seqs}}){
			while($scaffold =~ m/(N{$n_limit,})/g){  # 06Feb2017 replace 25 with $n_limit
		...
		$desc = "Average length of break (>=$n_limit N) between contigs in scaffold";  # 06Feb2017 replace 25 with $n_limit and change > to >=
	...
	##################################################################################
 	# 
	# N50 values #### 27Jan2016 JBH added N60-N90 reporting
	#
	...
	foreach my $length (reverse sort{$a <=> $b} @{$data{$type}{lengths}}){
		...
		while($running_total > int (($n_index / 100) * $total_size)){	
			...
			elsif ($n_index == 60 or $n_index == 70 or $n_index == 80 or $n_index == 90){
				printf "%${w}s %10d", "N" . $n_index . " $type length", $length;
				printf "%s %10d\n", "  L" . $n_index . " $type count", $i;
			}
...
#	$ng_values[100] = $min;
	...
	foreach my $base (qw (A C G T N)){
		...
		$desc = "$type %$base";
		printf "%${w}s %10s  number of %s %17d\n", $desc, $percent, $base, $bases{$base};
		store_results($desc, $percent) if ($csv);
